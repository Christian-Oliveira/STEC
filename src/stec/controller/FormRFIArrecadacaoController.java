...
public class FormRFIArrecadacaoController implements Initializable, IFormulario {

	Resposta resposta = new Resposta();
	FormRFIArrecadacao formulario = new FormRFIArrecadacao();
	...
	@FXML
	private JFXComboBox<String> cbControleMensal;
	@FXML
	private JFXComboBox<String> cbAutosInfracao;
	@FXML
	private JFXComboBox<String> cbAvaliacao;
        @FXML
        private Label lblControleMensal;
        @FXML
        private Label lblAutosInfracao;
	@FXML
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		cbAutosInfracao.getSelectionModel().selectedItemProperty().addListener((observable, oldValue,
				newValue) -> adicionarComentario(newValue, "Autos de infração"));
		cbControleMensal.getSelectionModel().selectedItemProperty().addListener((observable, oldValue,
				newValue) -> adicionarComentario(newValue, "Controle mensal de arrecadação"));
	}
	...
	private void handleAdicionar(ActionEvent event) {
		...
		formulario.setAutosInfracao(cbAutosInfracao.getSelectionModel().getSelectedItem());
		formulario.setControleMensal(cbControleMensal.getSelectionModel().getSelectedItem());
	...
	public void carregarComboboxOpcoes() {
		...
		cbAutosInfracao.setItems(observableOpcoes);
		cbControleMensal.setItems(observableOpcoes);
	...
	public void setResposta(Resposta resposta) {
		...
		if (resposta.getResposta() != null) {
			...
			FormRFIArrecadacao formulario = gson.fromJson(resposta.getResposta(), FormRFIArrecadacao.class);

			cbAutosInfracao.getSelectionModel().select(formulario.getAutosInfracao());
			cbControleMensal.getSelectionModel().select(formulario.getControleMensal());
	...
	public void setDialog(Stage dialog) {
        ...
        public boolean validarDados(){
            boolean error = false;
            if (cbControleMensal.getSelectionModel().isEmpty()){
                error = true;
                lblControleMensal.setStyle("-fx-text-fill:red");
            }
            if (cbAutosInfracao.getSelectionModel().isEmpty()){
                error = true;
                lblAutosInfracao.setStyle("-fx-text-fill:red");
            }
	...
	public class FormRFIArrecadacao {

		private String controleMensal;
		private String autosInfracao;
		...
		public String getControleMensal() {
			return controleMensal;
		}

		public void setControleMensal(String controleMensal) {
			this.controleMensal = controleMensal;
		}

		public String getAutosInfracao() {
			return autosInfracao;
		}

		public void setAutosInfracao(String autosInfracao) {
