...
public class FormAGOrganizacaoController implements Initializable, IFormulario {

    Resposta resposta = new Resposta();
    FormAGOrganizacao formulario = new FormAGOrganizacao();
    ...
    @FXML
    private JFXComboBox<String> cbArquivo;
    @FXML
    private JFXComboBox<String> cbMural;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        cbArquivo.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> adicionarComentario(newValue, "Arquivo"));
        ...
        cbMural.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> adicionarComentario(newValue, "Mural técnico"));
    }
    ...
    private void handleAdicionar(ActionEvent event) {
        if (validarDados()){
            ...
            formulario.setArquivo(cbArquivo.getSelectionModel().getSelectedItem());
            formulario.setFluxo(cbFluxo.getSelectionModel().getSelectedItem());
            formulario.setMural(cbMural.getSelectionModel().getSelectedItem());
    ...
    public void carregarComboboxOpcoes() {
        ...
        cbArquivo.setItems(observableOpcoes);
        cbFluxo.setItems(observableOpcoes);
        cbMural.setItems(observableOpcoes);
    ...
    public void setResposta(Resposta resposta) {
        ...
        if (resposta.getResposta() != null) {
            ...
            FormAGOrganizacao formulario = gson.fromJson(resposta.getResposta(), FormAGOrganizacao.class);

            cbArquivo.getSelectionModel().select(formulario.getArquivo());
            cbFluxo.getSelectionModel().select(formulario.getFluxo());
            cbMural.getSelectionModel().select(formulario.getMural());
    ...
    public boolean validarDados(){
        boolean error = false;
        if (cbArquivo.getSelectionModel().isEmpty()){
            error = true;
            lblArquivo.setStyle("-fx-text-fill:red");
        }
        if (cbMural.getSelectionModel().isEmpty()){
            error = true;
            lblMural.setStyle("-fx-text-fill:red");
        }
    ...
    public class FormAGOrganizacao {
