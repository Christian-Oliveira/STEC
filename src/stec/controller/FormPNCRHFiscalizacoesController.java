...
public class FormPNCRHFiscalizacoesController implements Initializable, IFormulario {
    
    Resposta resposta = new Resposta();
    FormPNCRHFiscalizacoes formulario = new FormPNCRHFiscalizacoes();
    ...
    @FXML
    private JFXComboBox<String> cbProcedimentoComprovacao;
    @FXML
    private JFXComboBox<String> cbRelatoriosCobertura;
    @FXML
    private JFXComboBox<String> cbAvaliacao;
    @FXML
    private Label lblProcedimentoComprovacao;
    @FXML
    private Label lblRelatoriosCobertura;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        cbProcedimentoComprovacao.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> adicionarComentario(newValue, "Procedimentos de comprovação de vacinação"));
        cbRelatoriosCobertura.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> adicionarComentario(newValue, "Relatórios de cobertura vacinal"));
    }    
    ...
    private void handleAdicionar(ActionEvent event) {
        if (validarDados()){
            ...
            formulario.setProcedimentosComprovacao(cbProcedimentoComprovacao.getSelectionModel().getSelectedItem());
            formulario.setRelatoriosCobertura(cbRelatoriosCobertura.getSelectionModel().getSelectedItem());
    ...
    public void carregarComboboxOpcoes(){
        ...
        cbProcedimentoComprovacao.setItems(observableOpcoes);
        cbRelatoriosCobertura.setItems(observableOpcoes);
    ...
    public void setResposta(Resposta resposta) {
        ...
        if (resposta.getResposta() != null) {
            ...
            FormPNCRHFiscalizacoes formulario = gson.fromJson(resposta.getResposta(), FormPNCRHFiscalizacoes.class);
            
            cbProcedimentoComprovacao.getSelectionModel().select(formulario.getProcedimentosComprovacao());
            cbRelatoriosCobertura.getSelectionModel().select(formulario.getRelatoriosCobertura());
    ...
    public boolean validarDados(){
        boolean error = false;
        if (cbProcedimentoComprovacao.getSelectionModel().isEmpty()){
            error = true;
            lblProcedimentoComprovacao.setStyle("-fx-text-fill:red");
        }
        if (cbRelatoriosCobertura.getSelectionModel().isEmpty()){
            error = true;
            lblRelatoriosCobertura.setStyle("-fx-text-fill:red");
        }
    ...
    public class FormPNCRHFiscalizacoes {
        
        private String procedimentosComprovacao;
        private String relatoriosCobertura;
        ...
        public String getRelatoriosCobertura() {
            return relatoriosCobertura;
        }

        public void setRelatoriosCobertura(String relatoriosCobertura) {
            this.relatoriosCobertura = relatoriosCobertura;
        }
